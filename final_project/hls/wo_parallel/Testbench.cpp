...
void Testbench::fetch_result() {
  ...
  for (y = 0; y != height; ++y) {
    for (x = 0; x != width; ++x) {
				for(int i=0; i<3; i++){
					if(i==0)
		  			*(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = i_result.get();
					else if(i==1)
		  			*(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = i_result.get();
					else
		  			*(target_bitmap + bytes_per_pixel * (width * y + x) + 0) = i_result.get();
    	}
		}
  }
