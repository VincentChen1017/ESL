...
class BasicOptions : public Options {
   public:
	...
	addr_t colorTransfer_P1_start_addr = 0x73000000;
	addr_t colorTransfer_P1_size = 0x00100000;
	addr_t colorTransfer_P1_end_addr = colorTransfer_P1_start_addr + colorTransfer_P1_size - 1;
	...
	void parse(int argc, char **argv) override {
		...
		assert((mem_end_addr < clint_start_addr || mem_start_addr > colorTransfer_P1_end_addr) &&
...
int sc_main(int argc, char **argv) {
	...
	COLORTRANSFER_P1 color_transfer_p1("color_transfer_p1");
	...
	bus.ports[12] = new PortMapping(opt.colorTransfer_P1_start_addr, opt.colorTransfer_P1_end_addr);
	// connect TLM sockets
	...
	bus.isocks[12].bind(color_transfer_p1.tsock);
