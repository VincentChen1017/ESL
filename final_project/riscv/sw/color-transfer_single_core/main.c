...
// COLOR_TRANSFER ACC
// phase1_R
static char* const COLORTRANSFER_P1_START_ADDR = (char* const)0x73000000;
static char* const COLORTRANSFER_P1_READ_ADDR = (char* const)0x73000004;
...
int main(int argc, char* argv[]) {
	...
	*/
	...
	union word data;
	union word data_ref;
	unsigned int sum_src_r = 0;
	unsigned int sum_ref_r = 0;
	unsigned int sum_src_g = 0;
	unsigned int sum_ref_g = 0;
	unsigned int sum_src_b = 0;
	unsigned int sum_ref_b = 0;
	...
	// printf("Start Processing Phase-1...(%d, %d)\n", width, height);
	...
	for (int y = 0; y < height; y++) {
		...
		for (int x = 0; x < width; x++) {
			...
			sum_src_r += w_buffer[0];
			// sum_ref_r += w_buffer_ref[0];
			...
			sum_src_g += w_buffer[1];
			// sum_ref_g += w_buffer_ref[1];
			...
			sum_src_b += w_buffer[2];
			// sum_ref_b += w_buffer_ref[2];

			write_data_to_ACC(COLORTRANSFER_P1_START_ADDR, w_buffer, 4);
			read_data_from_ACC(COLORTRANSFER_P1_READ_ADDR, r_buffer, 4);
			// printf("r_buffer[0]: %d\n", r_buffer[0]);
			// write_data_to_ACC(COLORTRANSFER_P1_START_ADDR, w_buffer_ref, 4);
			// read_data_from_ACC(COLORTRANSFER_P1_READ_ADDR, r_buffer_ref, 4);
