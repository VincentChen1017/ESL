...
// GAUSSBLUR Filter ACC
static char* const GAUSSBLURFILTER_START_ADDR = reinterpret_cast<char* const>(0x73000000);
static char* const GAUSSBLURFILTER_READ_ADDR = reinterpret_cast<char* const>(0x73000004);
			...
			for (int v = -1; v <= 1; v++) {
				for (int u = -1; u <= 1; u++) {
					if ((x + u) >= 0 && (x + u) < width && (y + v) >= 0 && (y + v) < height) {
						buffer[0] = *(source_bitmap + bytes_per_pixel * ((y + v) * width + (x + v)) + 2);
						buffer[1] = *(source_bitmap + bytes_per_pixel * ((y + v) * width + (x + v)) + 1);
						buffer[2] = *(source_bitmap + bytes_per_pixel * ((y + v) * width + (x + v)) + 0);
						buffer[3] = 0;
					} else {
						buffer[0] = 0;
						buffer[1] = 0;
					...
					write_data_to_ACC(GAUSSBLURFILTER_START_ADDR, buffer, 4);
				}
			}
			read_data_from_ACC(GAUSSBLURFILTER_READ_ADDR, buffer, 4);
