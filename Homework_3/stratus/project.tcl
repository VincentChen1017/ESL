...
define_hls_module GaussBlurFilter ../GaussBlurFilter.cpp

### 4. Define your HLS configuration (arbitrary names, BASIC and DPA in this example). 
define_hls_config GaussBlurFilter BASIC
define_hls_config GaussBlurFilter DPA       --dpopt_auto=op,expr
define_hls_config GaussBlurFilter UNROLL    --dpopt_auto=op,expr
define_hls_config GaussBlurFilter PIPELINE    --dpopt_auto=op,expr
define_hls_config GaussBlurFilter LATENCY    --dpopt_auto=op,expr
define_hls_config GaussBlurFilter UNROLL_PIPE    --dpopt_auto=op,expr
define_hls_config GaussBlurFilter LATENCY_PIPE    --dpopt_auto=op,expr
set IMAGE_DIR           ".."
...
define_sim_config V_BASIC "GaussBlurFilter RTL_V BASIC" -argv "$IN_FILE_NAME $OUT_FILE_NAME"
### 5.3 The Verilog simulation for HLS config "DPA". 
define_sim_config V_DPA "GaussBlurFilter RTL_V DPA" -argv "$IN_FILE_NAME $OUT_FILE_NAME"
### 5.4  
define_sim_config V_UNROLL "GaussBlurFilter RTL_V UNROLL" -argv "$IN_FILE_NAME $OUT_FILE_NAME"
### 5.5  
define_sim_config V_PIPELINE "GaussBlurFilter RTL_V PIPELINE" -argv "$IN_FILE_NAME $OUT_FILE_NAME"
### 5.6 
define_sim_config V_LATENCY "GaussBlurFilter RTL_V LATENCY" -argv "$IN_FILE_NAME $OUT_FILE_NAME"
### 5.7  
define_sim_config V_UNROLL_PIPE  "GaussBlurFilter RTL_V UNROLL_PIPE" -argv "$IN_FILE_NAME $OUT_FILE_NAME"
### 5.8  
define_sim_config V_LATENCY_PIPE  "GaussBlurFilter RTL_V LATENCY_PIPE" -argv "$IN_FILE_NAME $OUT_FILE_NAME"
